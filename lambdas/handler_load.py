...
def insert_data_into_customer_table(data,connection):
    ...
    with connection.cursor() as cursor:
        ...
        for name in customer_names:
            ...
            connection.commit()
            sql_command_select_customer_id = 'SELECT c.Customer_id FROM Customers AS c WHERE c.Forename = %s AND c.Surname = %s AND c.Customer_id = (SELECT MAX(k.Customer_id) FROM Customers AS k)'
